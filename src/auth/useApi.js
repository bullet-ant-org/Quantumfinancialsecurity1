...
const useApi = (apiCall) => {
  ...
  const request = useCallback(async (...args) => {
    console.log("useApi: Request started");
    setLoading(true);
    setError(null);
    try {
      const res = await apiCall(apiUrl, token, ...args);
      if (!res.ok) {
        console.log("useApi: Response not OK", res);
      ...
      console.log("useApi: API call successful, data:", resultData);
      setData(resultData);
      return { success: true, data: resultData };
    } catch (err) {
      console.error("API call failed:", err);
      console.log("useApi: API call failed, error:", err.message);
